...
describe('isFile', () => {
    describe('Check if directory is a file', () => {
        it('Should return false', async () => {
            const res = await isFile(join(TEST_DIR, 'lorem'));
    ...
    describe('Check if file is a file', () => {
        it('Should return true', async () => {
            const res = await isFile(join(TEST_DIR, 'lorem', 'ipsum', 'test.txt'));
            expect(res).toBe(true);
        });
    });
    ...
    describe('Check if not existing path returns false', () => {
        it('Should return false', async () => {
            const res = await isFile(join(TEST_DIR, Math.random().toString()));
            expect(res).toBe(false);
        });
    });
});
